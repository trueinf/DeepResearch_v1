...
async function executeNeo4jQuery(statements: Neo4jStatement[]): Promise<any> {
...
// Build graph in Neo4j using the schema: Concept, Person, Organization, Trend, Problem, Solution
async function buildNeo4jGraph(researchId: string, entities: Entity[], relationships: Relationship[]): Promise<void> {
  ...
  try {
    // Map entities to schema node types (Concept, Person, Organization, Trend, Problem, Solution)
    const nodeStatements = entities.map(entity => {
      const nodeType = mapEntityTypeToNodeType(entity.type)
      return {
        statement: `
          MERGE (n:${nodeType} {id: $id, researchId: $researchId})
          SET n.label = $label,
    ...
    // Map relationships to schema types (RELATES_TO, INFLUENCES, CAUSES, CONTRADICTS, SUPPORTS, PART_OF)
    ...
    for (let i = 0; i < nodeStatements.length; i += batchSize) {
      ...
      if (i + batchSize < nodeStatements.length) {
    ...
    for (let i = 0; i < relStatements.length; i += batchSize) {
      ...
      await executeNeo4jQuery(batch)
      if (i + batchSize < relStatements.length) {
        await delay(delayBetweenBatches)
      }
    }

    console.log(`Graph built in Neo4j: ${entities.length} nodes, ${relationships.length} relationships`)
  } catch (error) {
